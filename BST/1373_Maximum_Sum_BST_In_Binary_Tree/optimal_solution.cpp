...
public:
    ...
    // storing the <min, max, curr_sum> for each node. While traversing, we would want to evaluate this for every node and return
    // to it's parent. If we have a left<min, max, curr_sum> and right<min, max, curr_sum> for a node, this would means in what range the left
    ...
    // This is more like a postorder traversal, we go left->right and then process the root.
    std::vector<int> traverse(TreeNode *root, int &ans)
    {
        ...
        // Update the range.
        return {std::min(root->val, left[0]), std::max(root->val, right[1]), curr_sum};
    }

    int __maxSumBST(TreeNode *root)
    {
        int ans = 0;
        traverse(root, ans);
        return std::max(ans, 0); // We do not want to return -ve sum.
    }
};
