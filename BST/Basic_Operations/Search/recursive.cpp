...
{
public:
    // Search function that takes a node and the key that we are searching for in the tree.
...
int main()
{
    ...
    TreeNode *ans = op._search(node1, 6);
    if (ans)
    {
        std::cout << ans->val << "\n";
        std::cout << "Parent of " << ans->val << " is " << ans->parent->val << "\n";
    }
    return 0;
}
