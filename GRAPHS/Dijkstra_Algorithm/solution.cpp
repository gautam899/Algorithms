...
{
public:
    void find_shortest_path(int source, std::vector<std::vector<std::pair<int, int>>> &graph, std::vector<int> &dist, std::vector<int> &prev, std::vector<int> &visited)
    {
        dist[source] = 0;
        prev[source] = 0;
        ...
        while (!pq.empty())
        {
            ...
            for (auto neighbor : graph[node])
            {
                ...
                if (visited[neighbor.first] == 0 && (d + neighbor.second) < dist[neighbor.first])
                {
            ...
            visited[node] = 1; // Mark the node visited once all the neighbors are visited.
        }
    }
};
int main()
{
    ...
    sol.find_shortest_path(0, graph, dist, prev, visited);
